<!-- Self-contained Modus template-builder command — no external prompt file required.
     Sibling copies: .claude/commands/modus-template.md, .github/prompts/modus-template.prompt.md.
     Keep all three in sync when editing the body below. -->

# Modus template page from screenshot

You are a staff frontend engineer building a **copy-paste Modus template page**.

## Inputs

Read the template name, page title, any notes, and an optional verification level from the message that invoked this command. If the template name, page title, or the screenshot is missing, ask before proceeding rather than guessing.

- **Screenshot (required):** the image attached to this chat.
- **Template name:** e.g. Portal, Inbox, Project home.
- **Page title:** browser tab title + h1.
- **Optional notes:** anything the screenshot doesn't make obvious.
- **Verify level:** `disabled` / `minimal` / `high`, if the user specifies one. Default to `minimal` if unspecified. See **Browser verification level** below.
