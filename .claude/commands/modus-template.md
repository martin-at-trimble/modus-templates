...
<!-- Self-contained Modus template-builder command — no external prompt file required.
     Sibling copies: .cursor/commands/modus-template.md, .github/prompts/modus-template.prompt.md.
...
Parse `$ARGUMENTS` as pipe-separated: `<template name> | <page title> | [optional notes] | [verify: disabled|minimal|high]`.

- **Screenshot (required):** the image attached to this message.
- **Template name:** text before the first `|`.
- **Page title:** browser tab title + h1 — text between the first and second `|`.
- **Optional notes:** anything the screenshot doesn't make obvious — everything after the second `|` and before an optional fourth `|` (may be empty).
- **Verify level:** text after the fourth `|`, one of `disabled` / `minimal` / `high`. If omitted or unrecognized, default to `minimal`. See **Browser verification level** below.

If the template name or page title is missing, or no screenshot is attached, ask before proceeding rather than guessing.
